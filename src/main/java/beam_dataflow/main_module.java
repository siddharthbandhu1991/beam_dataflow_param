...
import beam_dataflow.DataflowSetup.*;
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		setPipelineOption options =
	            PipelineOptionsFactory.fromArgs(args).withValidation()
	              .as(setPipelineOption.class);
	    Pipeline pipeline = Pipeline.create(options);
	    
    
        String project = options.getProjectName().get();
        String bucket = options.getBucketName().get();
        String config_path = options.getConfigFilePath().get();
	    
		YamlUtil yamlUtil = new YamlUtil(project,bucket,config_path);
		
		String config_feeds = options.getFeedId().get();
		List<String> feedid = new ArrayList<String>();

		if (config_feeds.contains(",")) {
			String[] items = config_feeds.split(",");
			feedid = Arrays.asList(items);
		} else {
			if(config_feeds.equals("ALL"))
				feedid = yamlUtil.getallfeeds();
			else
				feedid.add(config_feeds);
		}
				
		@SuppressWarnings("rawtypes")
		ArrayList feeds = yamlUtil.getallfeeds();
		int feedcount = 0;
		while (feedid.size() > feedcount) {
			if(feeds.get(feedcount).toString().equals(feedid.get(feedcount)))
			{
